...
int main(int argc, char* argv[]) {
    ...
    bool jobFlag = true; // true: doing job,  false: done job
...
//                ("mc_mult", value<int>(&jobInput.mc_mult)->default_value(1), "MC step multiplier number (default value 1) [multiply MC steps] ")
                ...
                ;
        po::options_description runtime("Runtime Parameterization");
        runtime.add_options()
                ("threads,t", po::value<uint32_t>(&jobInput.cpu)->default_value(1), "number of threads per MPI rank (default value: 1)");
        ...
        catch (boost::exception_detail::clone_impl<boost::exception_detail::error_info_injector<po::unknown_option> > &msg) {
           parsingerror(msg.what());
        }
        // catching missing argument
        catch (boost::exception_detail::clone_impl<boost::exception_detail::error_info_injector<boost::program_options::invalid_command_line_syntax> > &msg) {
            parsingerror(msg.what());
        }
        // invalid option values
        catch (boost::exception_detail::clone_impl<boost::exception_detail::error_info_injector<boost::program_options::invalid_option_value> > &msg) {
            parsingerror(msg.what());
        }
        // catch multiple occurences of a command line option
        catch (boost::exception_detail::clone_impl<boost::exception_detail::error_info_injector<boost::program_options::multiple_occurrences> > &msg) {
            std::string message = " as a command line option";
